exports.run = async (bot, message, args) => {
    ...
    if(monsterHunter.includes(serverHours)){
        ...
        else if (serverHours > monsterHunter[1] && serverHours < monsterHunter[2] ) messageContent +=  `Monster Hunter happy hour starts in **${fixTime((monsterHunter[2]-1) - serverHours)}:${fixTime(59 - serverMinutes)}:${fixTime(59 - time.getSeconds())}**\n`
        else if (serverHours > monsterHunter[2] && serverHours < monsterHunter[3] ) messageContent +=  `Monster Hunter happy hour starts in **${fixTime((monsterHunter[3]-1) - serverHours)}:${fixTime(59 - serverMinutes)}:${fixTime(59 - time.getSeconds())}**\n`
        else if (serverHours > monsterHunter[3] && serverHours <= 23 ) messageContent += `Monster Hunter happy hour starts in **${fixTime((monsterHunter[3]+3) - serverHours)}:${fixTime(59 - serverMinutes)}:${fixTime(59 - time.getSeconds())}**\n`
    }
